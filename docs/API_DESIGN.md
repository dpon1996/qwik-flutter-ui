...
> **Status:** v1 layout + typography finalized. **v1.1** (§15–§21) specified — v1.1 open questions approved (§94). **v1.2** scrolling (§22–§24) specified — resolve open questions in §93 before implementation. **v1.25** `MediaQuery` specified (§25) — resolve open questions in §27 before implementation. **v1.3** forms **specified** (§28–§37). **v1.4** selection controls **specified** (§46–§56). **v1.5** theming **specified and implemented** (§57; decisions **T1–T6** in §97). **v1.6** form decoration **specified** (§58–§72; decisions **FD1–FD10** in §97). **v1.7** overlays **specified and implemented** (§73–§89; decisions **OV1–OV15** in §97). **v1.8** App Structure **specified** (§90–§91). **Next:** **v1.9 Navigation** (§98, §88).
...
- §91 — `AppShell`
- §92 — v1.2 scrolling — shared enums review
- §93 — v1.2 Scrolling open questions
- §94 — Open questions (v1.1, approval required)
- §95 — API consistency review
- §96 — Summary table
- §97 — Decisions log
- §98 — Roadmap (incl. version summary, scrolling, forms, theming)
...
Layout widgets (§3–§14) stay **non-interactive** — no `onClick$` (see §95.6). **`Button`** (§17) is the first widget that accepts Qwik event handlers.
...
| `AppBar` | Future export (v1.8) |
| `Drawer` | Future export (v1.8) |
| `SideSheet` | Future export (v1.8) |
| `BottomNavigationBar` | Future export (v1.8) |
...
| `fitWidth`    | `none` + sizing  | Scale down so width fits; height may clip. **Deferred in v1.1 impl (§94.5).** |
| `fitHeight`   | `none` + sizing  | Scale down so height fits; width may clip. **Deferred in v1.1 impl (§94.5).** |
...
| `Responsive<T>` (v2+, §95.7) | `Partial<Record<Breakpoint, T>>` or alias map |
...
**Not in v1.25:** `xs`–`xl` enum members — reserved for `Responsive<T>` string keys (§95.7). Additive enum members (e.g. `largeDesktop`) may ship in a future semver minor.
...
| `margin`          | —         | `EdgeInsets` (§0.3). Flutter default margin is `4` — **not** applied by default (§94.1). |
| `padding`         | —         | `EdgeInsets`.                                                                            |
...
export interface ButtonProps extends BaseProps, InteractiveProps {
  ...
  /** When set, renders navigation (see §94.2). */
  href?: string;
  target?: string;
  rel?: string;
...
| `href`            | —                       | Link navigation; see open questions §94.2.                    |
...
- Layout widgets remain non-interactive (§95.6); `Button` is the pattern for future `Link`, `IconButton`.
...
- **`BoxFit.fitWidth` / `fitHeight`** — enum members exist; full CSS mapping deferred in implementation (§94.5). Use `contain` / `cover` until then.
...
- Invalid `aspectRatio <= 0` — TypeScript should document; runtime: treat as `1` or dev-only warning (implementation choice, §94.7).
...
| `reverse`      | `false`           | Inner flex `column-reverse` / `row-reverse` (see §93 S4).             |
...
**We do not ship** a `scrollDirection` prop or `ScrollDirection` enum in v1.2. See §95.11 and §93 S2.

- **Scrollbars:** system/UA only in v1.2; custom `Scrollbar` → §98 future scrolling.
- **SSR:** outer `<div>` scrollport + inner wrapper; `overflow: auto`; no scroll listeners (Principles #4–#7).
- **Nested scroll:** caller responsibility; document in §93 S3.
...
| Version | Addition                                                                 |
| ------- | ------------------------------------------------------------------------ |
| v2      | `ScrollController`, `physics`, `primary`, `keyboardDismissBehavior` — §98 |
...
Scroll many sibling items (cards, settings rows, feeds) in one scrollport. v1.2 is **non-virtualized** (all children in the DOM) — suitable for moderate lists. For very long lists, defer to v2 builder/sliver APIs (§98).
...
export interface ListViewProps extends BaseProps {
   ...
   * Extension over Flutter (see §93 L5) — same idea as `Row.gap` / `Column.gap`.
   */
  gap?: Length;
  /**
...
**Default behavior:** Do **not** automatically apply `role="list"` or `role="listitem"` (§93 L1).
...
**`gap` (Flutter extension):** Flutter has no `gap` on `ListView`; spacing uses explicit widgets or `ListView.separated`. Documented in §93 L5 (same category as `Row.gap` in v1).
...
| **Milestone** | **v2** — §98 Future ListView enhancements |
...
| v2      | `ScrollController`, virtualization — §98                        |
...
**Spacing (G3):** `gap` + `mainAxisSpacing` — not `runSpacing`. `mainAxisSpacing` matches `SliverGridDelegate`; `gap` is the `crossAxisSpacing` analogue (§93 G3).
...
- **`columns` + `minItemWidth`:** both ship in v1.2; `columns` takes precedence (§93 G4).

### Future extensibility

| Version | Addition                                      |
| ------- | --------------------------------------------- |
| v2      | `SliverGrid`, builder delegates — §98         |
| v2      | `ScrollController`                            |
...
- Prerequisite for **`Responsive<T>`** (v2+, §95.7) and **`ThemeData`** per-tier tokens (v1.5+).
...
| `padding` / `viewPadding` | Defer | **`SafeArea` v2** (§98) |
...
**Classification:** Compare `width` to merged thresholds → one `Breakpoint` → derive booleans. Three tiers only on `MediaQueryData` (`mobile` / `tablet` / `desktop`). Finer `xs`–`xl` keys reserved for **`Responsive<T>`** v2+ (§95.7).
...
Flutter `SafeArea` reads `MediaQuery.padding` / `viewPadding`. v1.25 does **not** ship inset fields. Interim: `Container` + `padding: env(safe-area-inset-*)`. Ship **`SafeArea`** after v1.25 (§98).

#### Relationship to `Responsive<T>` (v2+)

v1.25: imperative `useMediaQuery()`. v2+: declarative props, e.g. `width?: Responsive<Length>` keyed by shared **`Breakpoint`** (§95.7). `xs`–`xl` optional on `Responsive<T>` only — not on `MediaQueryData`.
...
Resolve before v1.25 implementation. Record approvals in §97 decisions log.
...
- No `GlobalKey<FormState>` — Qwik context for registration (§95.13).
...
| Status | IDs | Notes |
| ------ | --- | ----- |
| **Approved** (§97 #73–#79) | F4, F6, F7, F8, F9, F11, F12 | Frozen in widget specs |
...
See §97 decisions **#73–#79** and §28–§31 widget specs.
...
| Status | IDs | Notes |
| ------ | --- | ----- |
| **Approved** (§97) | **SC2 (#80)**, **SC5 (#81)**, **SC6 (#82)**, **SC11 (#83)**, **SC12 (#84)** | Frozen in §46–§50 |
...
### SC2 — Standalone `Radio` vs `RadioGroup` only (**Approved**, §97 #80)
...
### SC5 — Switch API matches Checkbox (**Approved**, §97 #81)
...
See §97 decisions **#80–#84** and §46–§50 widget specs.
...
> **Status:** **Specified and implemented.** Decisions **T1–T6** approved (§97). Architecture open questions: **none**.
...
| `AppBar` | Defer | **v1.8** App Structure |
| `Drawer` | Defer | **v1.8** App Structure |
| `SideSheet` | Defer | **v1.8** App Structure |
| `BottomNavigationBar` | Defer | **v1.8** App Structure |
...
**Fully specified §90–§91 (AppShell only in phase 1).** See §90 (overview) and §91 (`AppShell` widget spec).

**`AppShell` is the foundational v1.8 widget.** `AppShell` is the Flutter `Scaffold` equivalent and serves as the root application structure component. `AppBar`, `Drawer`, `SideSheet`, and `BottomNavigationBar` are designed to integrate with `AppShell`; they may also be used independently where appropriate.
...
- `AppShell` — root scaffold; owns layout slots for chrome + main content
- `AppBar`
- `Drawer`
- `SideSheet`
- `BottomNavigationBar`
...
- Route management
- Responsive scaffold APIs — **future responsive milestone** (see below)
- Navigation widgets — **v1.9 Navigation**
...
| `Responsive<T>` | v2+ | Per-breakpoint props (§95.7) |
...
> **v1.7 overlays approved for ship** (implementation + hardening pass complete). Post-v1.7 roadmap updated in §98 and §88 — **v1.8 App Structure** (`AppShell`-centric), then **v1.9 Navigation**.
...
- Foundation for future chrome widgets (`AppBar`, `Drawer`, `SideSheet`, `BottomNavigationBar`) — **listed as integration targets only, no API in this phase**
...
| Optional chrome **slots** (future props) | Responsive breakpoints (`MediaQuery` §25 remains primitive) |
...
- SSR/resumability: no layout state in AppShell; props are inert JSX subtrees until sibling widgets exist
- Option A viable via `q:slot` (used internally in `Menu` §83) but adds slot-name coupling without Flutter equivalent

**Phase 1 scope note:** Only `children` (body) is specified in §91. Props `appBar`, `drawer`, `bottomNavigationBar`, `sideSheet` are **documented as reserved integration points** — types may appear in `AppShellProps` sketch as optional `JSX.Element` but **no widget specs** until follow-on sections.

> **Reserved props do not imply implementation approval.**

### 90.5 v1.8 phase scope

| In scope (this doc) | Out of scope (later sections) |
| ------------------- | ----------------------------- |
| `AppShell` layout + landmarks | `AppBar`, `Drawer`, `SideSheet`, `BottomNavigationBar` APIs |
...
Specification of future widgets requires separate design sections and approval.

Approval of §91 does not imply approval of:
...
Each widget requires its own design section, review, and approval before implementation.

### 90.6 Folder / export (pointer only)

Reference existing §0.10 `app-shell/` folder placeholder and public exports planning table — no implementation.
...
### 91.2 Recommended usage (phase 1)
...
Future shape (document target API, not sibling widgets):

```tsx
<AppShell
  appBar={/* <AppBar /> — future § */}
  drawer={/* <Drawer /> — future § */}
  bottomNavigationBar={/* <BottomNavigationBar /> — future § */}
>
  {/* body */}
</AppShell>
```

### 91.3 Future API shape (non-normative sketch)

The following interface illustrates intended AppShell integration points after AppBar, Drawer, SideSheet, and BottomNavigationBar are specified.

Only `children` is part of the approved phase-1 contract.

The remaining properties are architectural placeholders and must not be treated as approved runtime API.

The interface below is an architectural planning artifact.

Only `children` is approved for phase 1.

Reserved properties must not be treated as committed public API until their corresponding widget specifications are approved.
...
export interface AppShellProps extends BaseProps {
  ...
  /** Reserved — integrates when `AppBar` is specified. Not v1.8 phase 1. */
  appBar?: JSX.Element;
  /** Reserved — integrates when `Drawer` is specified. */
  drawer?: JSX.Element;
  /** Reserved — integrates when `SideSheet` is specified. */
  sideSheet?: JSX.Element;
  /** Reserved — integrates when `BottomNavigationBar` is specified. */
  bottomNavigationBar?: JSX.Element;
}
```

No new §1 enums in phase 1.
...
| `drawer` / `sideSheet` | `<aside>` | Complementary navigation; visibility/state owned by future widgets |
...
- No automatic `role` overrides on slotted chrome — future widgets own interactive a11y
...
| Client state | None in AppShell — drawer/nav state deferred |
| Resumability | Pass — no signals/context in v1.8 phase 1 |
...
| `Scaffold.appBar` / `drawer` / `bottomNavigationBar` | Same prop names; widgets deferred |
...
Resolve before v1.2 implementation. Record approvals in §97 decisions log.

| ID  | Widget / scope            | Topic                                      | Recommendation                          |
| --- | ------------------------- | ------------------------------------------ | --------------------------------------- |
| S1  | `SingleChildScrollView`   | Scrollbar styling                          | **(A)** System scrollbars only in v1.2  |
| S2  | All scrolling             | `axis` vs `scrollDirection`                | **(A)** `axis` only — §95.11            |
...
**Recommendation:** **(A)** System/UA scrollbars only. Custom `Scrollbar` widget deferred to v2 (§98).

### S2 — `axis` vs `scrollDirection`

**Question:** Reuse `axis` or introduce `scrollDirection` / `ScrollDirection`?

**Recommendation:** **(A)** `axis` only. See §22–§24 Flutter parity note and §95.11.
...
## 94. Open questions (approval required)

All items **approved** — recorded in §97 decisions #32–42. Retained here as an audit trail. Implementation PRs for v1.1 may proceed.
...
### 94.11 Align implementation strategy
...
### 95.2 Decisions encoded since the last review
...
### 95.3 [RESOLVED] Semantic HTML `as` prop on `Text`
...
### 95.4 [RESOLVED] Semantic HTML `as` prop on `Container`
...
### 95.5 [RESOLVED] `Wrap.spacing` / `Wrap.runSpacing` naming asymmetry with `Row.gap` / `Column.gap`
...
### 95.6 [UPDATED] Event handlers on layout widgets

**Gap:** Layout widgets have no `onClick$`. A clickable card must not use `onClick$` on `Card`/`Container`.

**Resolution:** **Unchanged for layout widgets.** `Button` (§17) is the v1.1 interactive primitive; `Link` / `Tappable` remain v2 (§98). `Card` stays presentational — wrap with `Button` or use `Button` with `href` for navigation.

### 95.7 [RESOLVED] Responsive prop shape (deferred to v2)
...
### 95.8 [RESOLVED] `Length` accepts arbitrary strings
...
### 95.9 [NEW] v1.1 basic UI + layout consistency
...
### 95.10 [NEW] v1.2 scrolling consistency

| Check | Status | Notes |
| ----- | ------ | ----- |
| Flutter parity primary | Pass | `mainAxisSpacing`, `columns`, `childAspectRatio`; `gap` as `crossAxisSpacing` analogue. |
| `axis` not `scrollDirection` | Pass | §95.11; Flutter parity note in §22–§24. |
| No new enums in v1.2 | Pass | §92 defers `ScrollDirection`, `ScrollPhysics`, `ScrollBehavior`. |
| CSS Grid documented | Pass | §24 CSS mapping table. |
| Deferred APIs separated | Pass | v1.2 vs v2 in §23 Notes and §95.10. |
| Accessibility | Pass | No default list/grid roles (§93 L1). |
| SSR | Pass | CSS `overflow` / grid only in v1.2. |

### 95.11 [NEW] Scrolling widgets — `axis` vs Flutter `scrollDirection`
...
### 95.12 [NEW] v1.25 MediaQuery consistency
...
### 95.13 [NEW] v1.3 forms consistency
...
### 95.14 [NEW] v1.4 selection controls consistency
...
v1 decisions (#1–31) resolved. **v1.1 open questions** (#32–42) approved in §94.
...
| 32 | **`Card.margin` default** (§94.1)                                                                 | **No default margin** — caller opts in; document Flutter `margin: 4` in examples. |
| 33 | **`Button` + `href`** (§94.2)                                                                     | **Automatically render `<a>`** when `href` is set (no `as="a"` required). |
| 34 | **`ButtonVariant.elevated` in v1.1** (§94.3)                                                      | **Ship `elevated`** alongside `filled`, `outlined`, `text`.               |
| 35 | **`Visibility` hidden behavior** (§94.4)                                                          | **Apply `inert`** (and `pointer-events: none`) when `visible={false}`.   |
| 36 | **`Image` `BoxFit.fitWidth` / `fitHeight`** (§94.5)                                               | **Deferred** — v1.1 ships `fill`, `contain`, `cover`, `none`, `scaleDown` only. |
| 37 | **v1.1 release grouping** (§94.6)                                                                 | **Single milestone** — all seven v1.1 widgets; implement `Button` last.  |
| 38 | **`AspectRatio` invalid ratio** (§94.7)                                                            | **Clamp to `1`** and emit **dev warning** in development builds.          |
| 39 | **`Button` loading state** (§94.8)                                                                | **Defer `loading` prop to v1.2** — keep v1.1 `Button` minimal.            |
| 40 | **`Image` default `loading`** (§94.9)                                                             | **Default `ImageLoading.lazy`** (`loading="lazy"`); heroes opt into `eager`. |
| 41 | **`Card` default semantic tag** (§94.10)                                                          | **Default `as="div"`**; consumers use `as="article"` when appropriate.  |
| 42 | **`Align` implementation** (§94.11)                                                               | **CSS Grid** (`place-items`) — not absolute positioning.                  |
...
| 55 | **Scrolling widgets**                                                                             | Specified §22–§24; implement after §93 open questions resolved.           |
| 56 | **Forms + selection + theming**                                                                   | v1.3 forms §28–§37; v1.4 selection §46–§56; v1.5 theming §57.              |
| 57 | **Scrolling `axis` vs `scrollDirection`** (§93 S2, §95.11)                                        | **`axis` only** — no alias, no `ScrollDirection` enum.                    |
| 58 | **`ListView` ARIA roles** (§93 L1)                                                                | **No automatic** `role="list"` / `listitem"`.                            |
| 59 | **`ListView.gap` in v1.2** (§93 L5)                                                               | **Ship `gap`** — documented Flutter extension.                          |
| 60 | **`GridView` spacing** (§93 G3)                                                                   | **`gap` + `mainAxisSpacing`** — not `runSpacing`.                       |
| 61 | **`GridView` `columns` + `minItemWidth`** (§93 G4)                                                | **Ship both**; `columns` wins when both set.                            |
...
Fully specified. Decisions **FD1–FD10** approved (§97). Architecture questions **closed** (§69).
...
Fully specified and **implemented**. Decisions **OV1–OV15** approved (§97). Architecture questions **closed** (§86).
...
**Components:** `AppShell`, `AppBar`, `Drawer`, `SideSheet`, `BottomNavigationBar`
...
- `Responsive<T>` — v2+ (§95.7)

#### Future (v2+)
- `SafeArea`, `ScrollController`, `PageView`, `CustomScrollView`, `SliverList`, `SliverGrid`, `Scrollbar` — §98
- `Responsive<T>`, `BreakpointProvider`, `ResponsiveVisibility`, `ResponsiveValue<T>`, `AdaptiveScaffold` — §98
...
1. Resolve open questions in §94.
...
Fully specified in **§22–§24**. Resolve open questions in **§93** before implementation.

**Implementation sequence (v1.2 — after v1.1):**

1. Resolve open questions in §93.
...
| `ResponsiveValue<T>` | Future | Not v1.8 |
| `AdaptiveScaffold` | Future | Not v1.8; distinct from `AppShell` |
| `Responsive<T>` | v2+ | Per-breakpoint props without `MediaQuery` branches (§95.7, Principle #9) |
...
> **Gate:** §94 v1.1 open questions approved.
...
> **Gate:** §93 v1.2 scrolling open questions resolved.
...
> **Spec:** §57. Decisions **T1–T6** approved (§97).
...
> **Spec:** §58–§72. Decisions **FD1–FD10** approved (§97).
...
> **Spec:** §73–§89. Decisions **OV1–OV15** approved (§97).
...
> **Spec:** §90–§91. Phase 1: `AppShell` only — no `AppBar`, `Drawer`, `SideSheet`, or `BottomNavigationBar` in this phase.

- [ ] `src/lib/app-shell/` — `AppShellProps` from §91 (`children` only; reserved props documented, not implemented until sibling widgets specified).
- [ ] Semantic layout: root wrapper + `<main>` for body; SSR-safe structural markup (§91.4–§91.7).
